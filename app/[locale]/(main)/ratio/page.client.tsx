		...
		<div className="flex flex-col items-center justify-center gap-6 overflow-auto p-2">
			...
			<Popover open={open} onOpenChange={setOpen}>
				<PopoverTrigger asChild>
					<Button
						className="h-12 min-w-[200px] justify-between border-transparent bg-secondary p-2 capitalize shadow-md"
						title=""
						role="combobox"
						variant="outline"
					>
...
function RequirementCard({ input }: RequirementCardProps) {
	...
	const filtered = blocks.filter(
		(block) => block.name.includes(filter) && block.output.some((output) => output.name === input.name),
	);
