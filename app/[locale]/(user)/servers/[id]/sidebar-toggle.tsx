  ...
  return (
    <div className="flex h-9 w-full items-center justify-center">
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger
          aria-expanded={open}
          className={cn('flex w-full flex-row items-center justify-between overflow-hidden p-0 pl-2 font-bold', {
            'w-0 p-0': !visible,
          })}
        >
          {value ? <ColorText className="text-lg font-bold" text={servers.find((server) => server.id === value)?.name} /> : ''}
          <CaretSortIcon className="ml-2 h-4 w-4 shrink-0 opacity-50" />
        </PopoverTrigger>
        <PopoverContent className="grid w-[200px] gap-1 p-2">
          {servers.map(({ id, name }) => (
            <InternalLink key={id} href={pathname.replace(serverId, id)} onClick={() => setValue(id)}>
