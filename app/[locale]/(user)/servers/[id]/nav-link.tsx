...
export default function NavLink({ id, href, label, icon, hovered, setHovered }: Props) {
  ...
  return (
    <InternalLink
      className={cn('relative inline-flex h-12 items-center justify-center gap-2 text-nowrap px-0 py-2 text-sm text-foreground/70 hover:text-foreground', {
    ...
    >
      <div className="relative w-full">
        ...
        >
          <div className="relative flex justify-center w-fit gap-1 rounded-sm">
            <span>{icon}</span>
            <span>{label}</span>
          </div>
        </div>
