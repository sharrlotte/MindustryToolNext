  ...
  ({ className, children, showChevron = true, ...props }, ref) => (
    <AccordionPrimitive.Header className="flex">
      <AccordionPrimitive.Trigger ref={ref} className={cn('flex flex-1 items-center justify-between py-4 text-sm font-medium transition-all group', className)} {...props}>
        {children}
        {showChevron && <ChevronDownIcon className="h-4 w-4 ml-auto mr-2 shrink-0 transition-transform duration-200 group-aria-expanded:rotate-180 dark:text-foreground dark:hover:text-foreground" />}
