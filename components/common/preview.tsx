...
export function Preview({ className, children, ...props }: CardProps) {
  return (
    <div className={cn('group overflow-hidden relative flex min-h-preview-height min-w-[min(100vw,var(--preview-size))] bg-card/50 max-w-[calc(var(--preview-size)*2)] flex-col rounded-lg shadow-md animate-appear', className)} {...props}>
      {children}
    </div>
  );
}
...
export function PreviewImage({ className, src, errorSrc, alt, ...props }: ImageProps) {
  return <FallbackImage className={cn('aspect-square object-cover w-full rounded-t-lg bg-black', className)} src={src} errorSrc={errorSrc} alt={alt} width={224} height={224} priority={props.loading == 'eager' ? true : undefined} {...props} />;
}
